...
import torch
from customized.mixup import mixup_bce, mixup_two_targets

# customized modules
from dataTools.factory import create_dataset, create_pu_dataset
...
from utils import fix_random_seed, train, validate
...
def get_params():
    parser = argparse.ArgumentParser(
        description="Dist-PU: Positive-Unlabeled Learning From a Label Distribution Perspective"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="cifar-10",
        choices=["cifar-10", "fmnist", "alzheimer"],
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="num of labeled positives in training set",
    )
...
def warm_up():
    for epoch in range(args.warm_up_epochs):
        ...
        print(
            "epoch:{}; loss: {:.4f}, {:.4f}; \
            acc: {:.5f}; precision: {:.5f}; recall: {:.5f}, f1: {:.5f}, auc: {:.5f}, ap: {:.5f}, lr: {:.5f}".format(
                epoch, train_loss, test_loss, acc, precision, recall, f1, auc, ap, optimizer.param_groups[0]["lr"]
            )
        )
